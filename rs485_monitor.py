...
class SerialMonitorApp:
    ...
    def _update_graph_ylabel_now(self):
        ...
        names = self._checked_field_names()
        if names and all("kW" in n for n in names):
    ...
    def _update_graph(self):
        if not self.graph_paused and self.time_buffer:
            ...
            xs_ds = xs[::step]
            any_plotted = False
            checked_names = self._checked_field_names()
            for i, var in enumerate(self.field_vars):
                if var.get() and i < len(self.field_buffers):
                    ...
                    if n > 0:
                        ...
                        any_plotted = True
            self.ax.set_xlabel("Time (s)")
            if checked_names and all("kW" in n for n in checked_names):
                self.ax.set_ylabel("Power (kW)")
            else:
                self.ax.set_ylabel("Value (decimal)")
