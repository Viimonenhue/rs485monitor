...
APP_VERSION = "1.3"   # bump this each time I hand you a new file
...
def field_scale_info(name):
    ...
    - Strom A (current) fields: raw value is in units of 0.1 A -> divide by
      10, capped at 20 A.
    - Encoder fields: raw value is an absolute count -> converted to degrees
      assuming a 2048-count/revolution encoder (360/2048 per count). Flag
      me if the real encoder resolution turns out to be different.
...
class SerialMonitorApp:
    ...
    def _build_graph_tab(self, notebook):
        ...
        ttk.Button(controls, text="Export Graph...", command=self._export_graph).pack(side="right", padx=5)
        ...
        self.belastningsvakt_val = tk.StringVar(value="1,99")
        ...
        ttk.Checkbutton(threshold_row, text="Motorskydd", variable=self.motorskydd_var,
                         command=self._redraw_now).pack(side="left")
        self.motorskydd_val = tk.StringVar(value="1,0")
    ...
    def _style_axes(self):
        ...
        units = self._checked_units()
        if len(units) == 1:
            step = GRID_STEP_BY_UNIT.get(next(iter(units)))
            if step:
    ...
    def _draw_threshold_lines(self):
        """Draws optional dashed threshold lines - only on the graph types
        they're relevant to. Demonstration lines only, not live data."""
        units = self._checked_units()

        if self.belastningsvakt_var.get() and "kW" in units:
            val = self._parse_number(self.belastningsvakt_val.get())
            if val is not None:
                self.ax.axhline(val, linestyle="--", color="#d35400", linewidth=1.3,
                                 label=f"Belastningsvakt ({val:g} kW)")

        if self.personskydd_var.get() and "kW" in units:
            val = self._parse_number(self.personskydd_val.get())
            if val is not None:
                self.ax.axhline(val + 0.25, linestyle="--", color="#8e44ad", linewidth=1.3,
                                 label=f"Personskydd ({val:g} kW \u00b1)")
                self.ax.axhline(val - 0.10, linestyle="--", color="#8e44ad", linewidth=1.3,
                                 label="_nolegend_")

        if self.motorskydd_var.get() and "A" in units:
            val = self._parse_number(self.motorskydd_val.get())
            if val is not None:
                self.ax.axhline(val * 1.5, linestyle="--", color="#16a085", linewidth=1.3,
                                 label=f"Motorskydd ({val:g} A \u00b150%)")
                self.ax.axhline(val * 0.5, linestyle="--", color="#16a085", linewidth=1.3,
                                 label="_nolegend_")

    def _draw_graph(self):
        """Draws the current graph state. Does NOT reschedule itself - call
        this directly for an immediate redraw (e.g. threshold checkbox
        toggled), or via _update_graph() on the periodic timer."""
        ...
        self._style_axes()
        self._draw_threshold_lines()
        if any_plotted or self.belastningsvakt_var.get() or self.personskydd_var.get() or self.motorskydd_var.get():
