...
APP_VERSION = "0.16"   # bump this each time I hand you a new file
...
class SerialMonitorApp:
    ...
    @staticmethod
    def _snap_to_extrema(raw_runs, vals, window):
        """Adjusts each close<->open (or open<->close) boundary between
        two adjacent runs to land exactly on the true local min/max of
        the RAW signal within a window around that boundary, instead of
        wherever the smoothed-derivative detection happened to catch up
        to the real reversal (which always lags a bit behind it)."""
        if len(raw_runs) < 2:
            return raw_runs
        refined = list(raw_runs)
        for i in range(len(refined) - 1):
            s1, e1, d1 = refined[i]
            s2, e2, d2 = refined[i + 1]
            ...
            if hi <= lo:
                continue
            candidates = [k for k in range(lo, hi + 1) if vals[k] == vals[k]]  # skip NaN
            if not candidates:
                continue
            if d1 < 0 and d2 > 0:
                boundary = min(candidates, key=lambda k: vals[k])  # closing then opening -> true minimum
            else:
                boundary = max(candidates, key=lambda k: vals[k])  # opening then closing -> true maximum
            refined[i] = (s1, boundary, d1)
            refined[i + 1] = (min(boundary + 1, e2), e2, d2)
        return refined

    def _compute_direction_runs(self, encoder_idx=0):
        """Returns a list of (start_time, end_time, kind) spans describing
        the given encoder's direction of travel - 'open' (increasing),
        'close' (decreasing), or 'reversing' (a quick flip shorter than
        the filter duration, only in that mode). encoder_idx=0 is
        Encoder 1 (M1), encoder_idx=1 is Encoder 2 (M2). Wrap-aware: a
        jump across the 359->0 boundary is treated as the small real
        movement it is, not a huge fake swing. Spans are sequential and
        never overlap, since each moment in time belongs to exactly one
        run.
        ...
        # The smoothing above is deliberately lagged - it takes a bit of
        # sustained movement in the new direction before the smoothed
        # derivative clearly crosses the threshold, so the raw boundary
        # above sits some distance AFTER the real turning point. Snap
        # each close<->open boundary to wherever the RAW encoder value
        # actually bottoms out (or peaks) in a window around that
        # boundary - the true reversal is exactly there, not wherever the
        # smoothed trend happened to catch up to it.
        raw_runs = self._snap_to_extrema(raw_runs, vals, window)
    ...
    def _draw_graph(self):
        ...
        try:
            self._remove_extra_axes()
            self.ax.clear()
            # ax.clear() silently wipes every callback registered on this
            # axes, including the xlim_changed one that detects a real
            # user zoom/pan - without reconnecting it here, _user_zoomed
            # could never become True again after the very first redraw,
            # so nothing ever got preserved (any zoom would pop right back
            # out on the next periodic update).
            self.ax.callbacks.connect("xlim_changed", self._on_xlim_changed)
            self.ax.set_xlabel("Time (s)")
            ...
            # Movement direction indicator - M1 and M2 tracked
            # independently (Encoder 1 / Encoder 2), since the two motors
            # can genuinely be moving in different directions at once.
            # Drawn as two thin strips near the bottom rather than
            # full-height spans, so M1 and M2 never visually overlap or
            # blend into each other even when they disagree.
            if self.direction_mode_var.get() != DIRECTION_MODES[0]:
                ...
                seen_kinds = set()
                # (encoder_idx, motor_label, strip ymin, strip ymax) - axes fraction
                strips = [(0, "M1", 0.00, 0.045), (1, "M2", 0.045, 0.09)]
                for enc_idx, motor_label, y0, y1 in strips:
                    for start, end, kind in self._compute_direction_runs(enc_idx):
                        lbl = f"{motor_label} {span_labels[kind]}" if (motor_label, kind) not in seen_kinds else "_nolegend_"
                        seen_kinds.add((motor_label, kind))
                        span = self.ax.axvspan(start, end, ymin=y0, ymax=y1, color=span_colors[kind],
                                                alpha=0.65, zorder=0, label=lbl)
                        if lbl != "_nolegend_":
                            all_handles.append(span)
                            all_labels.append(lbl)
                    self.ax.text(0.002, (y0 + y1) / 2, motor_label, transform=self.ax.transAxes,
                                 fontsize=6.5, va="center", ha="left", color="#555", zorder=1)
            ...
            # Y-axis scale/ticks - done as a separate pass (not inline
            # above) so M1 and M2 of the same physical unit can be sized
            # together below, rather than each just autoscaling to its
            # own data independently.
            grid_ref_ax = None  # first kW/A axis found - Encoder never wins this, no matter its position
            handled = set()

            # M1 and M2 share the exact same Y-scale whenever both their
            # kW (or both their Amp) axes are shown, so a taller-looking
            # line always means a genuinely bigger value, never just an
            # artifact of two independently autoscaled axes.
            for idx_a, idx_b in ((self.IDX_KW_M1, self.IDX_KW_M2), (self.IDX_A_M1, self.IDX_A_M2)):
                present = [i for i in (idx_a, idx_b) if i in axis_by_index]
                if not present:
                    continue
                ymax_candidates = []
                combined_anchors = []
                for i in present:
                    ...
                    _, ymax_i = ax.get_ylim()
                    ymax_candidates.append(ymax_i)
                    combined_anchors.extend(threshold_anchors.get(i, []))
                shared_ymax = max(ymax_candidates) if ymax_candidates else 1.0
                for i in present:
                    ax = axis_by_index[i]
                    _, unit, _ = field_scale_info(field_label(i))
                    ax.set_ylim(0, shared_ymax)
                    ax.set_yticks(self._nice_ticks(0, shared_ymax, anchors=combined_anchors))
                    ax.yaxis.set_major_formatter(FuncFormatter(self._tick_formatter_for_unit(unit)))
                    handled.add(i)
                    if grid_ref_ax is None and unit in ("kW", "A"):
                        grid_ref_ax = ax

            # Everything else (Encoder, or a kW/A field checked without
            # its motor pair) just autoscales to its own data as before.
            for i, ax in axis_by_index.items():
                if i in handled:
                    continue
                _, unit, _ = field_scale_info(field_label(i))
                if unit is not None:
