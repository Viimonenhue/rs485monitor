...
APP_VERSION = "0.11"   # bump this each time I hand you a new file
...
class SerialMonitorApp:
    ...
    def _build_menu(self):
        ...
        settings_menu.add_command(label="Display Batching...", command=self._open_settings_dialog)
    ...
    def _build_graph_tab(self, notebook):
        ...
        ttk.Button(overlay_row, text="Clear Overlays", command=self._clear_overlays).pack(side="left")
    ...
    # How many gridlines every Y-axis shares. Different axes have wildly
    # different ranges (degrees vs kW vs Amps), so "nice round numbers"
    # per axis never lines up between them. Using the SAME tick count,
    # evenly spaced across each axis's own range, makes every axis land on
    # the same pixel rows instead - so the grid is genuinely shared rather
    # than each axis drawing its own independent set of lines.
    GRID_TICK_COUNT = 8
    ...
    def _draw_threshold_lines(self, axis_by_index, color_by_index):
        ...
        if pk_idx is not None and self.personskydd_var.get():
            ...
            if val is not None:
                ax.axhline(val + 0.25, linestyle=":", color=color, linewidth=1.6,
                           label=f"Personskydd ({val:g} kW \u00b1)")
                ax.axhline(val - 0.10, linestyle=":", color=color, linewidth=1.6,
                           label="_nolegend_")
    ...
    def _draw_graph(self):
        ...
        try:
            ...
            for pos, i in enumerate(checked):
                ...
                # Every axis gets the SAME number of ticks, evenly spaced
                # across its own range. Since all these axes occupy the
                # same plot rectangle, tick N on one axis lands on exactly
                # the same pixel row as tick N on every other axis - so
                # the grid (drawn once, on the primary axis) actually lines
                # up with every axis's numbers instead of missing them.
                ymin, ymax = ax.get_ylim()
                ax.set_yticks(self._linspace(ymin, ymax, self.GRID_TICK_COUNT))
            ...
            # primary axis's own evenly-spaced ticks set above, which every
            # other axis's ticks are now aligned to.
            self.ax.grid(True, linewidth=0.3, alpha=0.6)
