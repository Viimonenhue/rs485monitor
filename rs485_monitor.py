...
APP_VERSION = "1.4"   # bump this each time I hand you a new file
...
# Y-axis grid/tick spacing on the graph, keyed by unit.
GRID_STEP_BY_UNIT = {
    "kW": 0.05,
    "A": 0.1,
}
...
class SerialMonitorApp:
    ...
    def _rebuild_table_columns(self):
        ...
        for i in range(self.field_count):
            key = f"f{i}"
            self.log_tree.column(key, width=115, anchor="center", stretch=True)
    ...
    def _build_graph_tab(self, notebook):
        ...
        ttk.Label(threshold_row, text="Thresholds:", font=("Segoe UI", 9, "bold")).pack(side="left", padx=(0, 8))

        # Belastningsvakt (single line, kW graphs only)
        self.belastningsvakt_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(threshold_row, text="Belastningsvakt", variable=self.belastningsvakt_var,
                         command=self._redraw_now).pack(side="left")
        self.belastningsvakt_val = tk.StringVar(value="0,70")
        ttk.Entry(threshold_row, textvariable=self.belastningsvakt_val, width=6).pack(side="left", padx=(2, 4))
        ttk.Label(threshold_row, text="kW").pack(side="left", padx=(0, 14))

        # Personskydd (two lines: +0.25 / -0.10, kW graphs only)
        self.personskydd_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(threshold_row, text="Personskydd", variable=self.personskydd_var,
                         command=self._redraw_now).pack(side="left")
        self.personskydd_val = tk.StringVar(value="0,20")
        ttk.Entry(threshold_row, textvariable=self.personskydd_val, width=6).pack(side="left", padx=(2, 4))
        ttk.Label(threshold_row, text="kW (+0.25 / -0.10)").pack(side="left", padx=(0, 14))

        # Motorskydd (two lines: +/-50%, Ampere graphs only)
        self.motorskydd_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(threshold_row, text="Motorskydd", variable=self.motorskydd_var,
                         command=self._redraw_now).pack(side="left")
        self.motorskydd_val = tk.StringVar(value="0,8")
        ttk.Entry(threshold_row, textvariable=self.motorskydd_val, width=6).pack(side="left", padx=(2, 4))
        ttk.Label(threshold_row, text="A (\u00b150%)").pack(side="left")
    ...
    def _clear_graph(self):
        ...
        self.ax.clear()
        self.ax.set_xlabel("Time (s)")
        self._draw_threshold_lines_multi({})
    ...
    def _draw_threshold_lines_multi(self, axes_by_unit):
        """Draws optional dashed threshold lines on whichever axis (or axes)
        match their unit. axes_by_unit maps unit -> list of axes currently
        showing that unit. Demonstration lines only, not live data."""
        kw_axes = axes_by_unit.get("kW", [])
        a_axes = axes_by_unit.get("A", [])

        if self.belastningsvakt_var.get() and kw_axes:
            val = self._parse_number(self.belastningsvakt_val.get())
            if val is not None:
                for n, ax in enumerate(kw_axes):
                    ax.axhline(val, linestyle="--", color="#d35400", linewidth=1.3,
                               label=f"Belastningsvakt ({val:g} kW)" if n == 0 else "_nolegend_")

        if self.personskydd_var.get() and kw_axes:
            val = self._parse_number(self.personskydd_val.get())
            if val is not None:
                for n, ax in enumerate(kw_axes):
                    ax.axhline(val + 0.25, linestyle="--", color="#8e44ad", linewidth=1.3,
                               label=f"Personskydd ({val:g} kW \u00b1)" if n == 0 else "_nolegend_")
                    ax.axhline(val - 0.10, linestyle="--", color="#8e44ad", linewidth=1.3,
                               label="_nolegend_")

        if self.motorskydd_var.get() and a_axes:
            val = self._parse_number(self.motorskydd_val.get())
            if val is not None:
                for n, ax in enumerate(a_axes):
                    ax.axhline(val * 1.5, linestyle="--", color="#16a085", linewidth=1.3,
                               label=f"Motorskydd ({val:g} A \u00b150%)" if n == 0 else "_nolegend_")
                    ax.axhline(val * 0.5, linestyle="--", color="#16a085", linewidth=1.3,
                               label="_nolegend_")
    ...
    def _draw_graph(self):
        ...
        step, and threshold lines on whichever axes match their unit.
        Does NOT reschedule itself - call directly for an immediate redraw
        (e.g. a checkbox toggled), or via _update_graph() on the timer."""
        ...
        for pos, i in enumerate(checked):
            color = self.AXIS_COLORS[pos % len(self.AXIS_COLORS)]
            ...
            if n > 0:
                line, = ax.plot(xs_ds[:n], ys[:n], color=color, label=field_label(i))
            ...
            axes_by_unit.setdefault(unit, []).append(ax)

        # Single shared grid, on the primary axis only, at the finest step
        # among the checked units (so kW+A together don't draw two grids).
        self.ax.grid(True, linewidth=0.3, alpha=0.6)
        steps = [GRID_STEP_BY_UNIT[u] for u in axes_by_unit if u in GRID_STEP_BY_UNIT]
        if steps:
            self.ax.yaxis.set_major_locator(MultipleLocator(min(steps)))

        self._draw_threshold_lines_multi(axes_by_unit)
    ...
    def _redraw_now(self):
        """Immediate redraw, e.g. after toggling a threshold checkbox."""
        self._draw_graph()
    ...
    def _save_xlsx_with_values(self):
        ...
        for ts, line, fields in self.raw_rows:
            ...
            for i in range(max_fields):
                ...
                if unit is not None:
                    scale, _u, cap = field_scale_info(field_label(i))
                    try:
                        val = int(hex_str, 16) * scale
                        if cap is not None and val > cap:
                            val = cap
                    except ValueError:
                        val = ""
