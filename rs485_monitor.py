...
MAX_GRAPH_POINTS = 500          # how many samples stay visible on the graph
...
# Excel breakdown). Positions not given a name yet just show as "Field N".
FIELD_NAMES = [
    "Enc M1",           # A
    "Enc M2",           # B
    "Effekt kW M1",     # C
...
class SerialMonitorApp:
    def __init__(self, root):
        ...
        self.field_buffers = []        # deque per field, holds recent values
        self.time_buffer = deque(maxlen=MAX_GRAPH_POINTS)
        self.sample_index = 0
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(log_tab, text="Auto-scroll", variable=self.autoscroll_var).pack(anchor="w", padx=5, pady=(5, 0))

        log_frame = ttk.Frame(log_tab)
        log_frame.pack(fill="both", expand=True, padx=5, pady=5)
        self.log_text = tk.Text(log_frame, wrap="none", font=("Consolas", 10))
    ...
    def _build_field_checkboxes(self):
        ...
        for i in range(self.field_count):
            ...
            self.field_buffers.append(deque(maxlen=MAX_GRAPH_POINTS))
        ...
        if not self.graph_paused and self.time_buffer:
            ...
            for i, var in enumerate(self.field_vars):
                if var.get() and i < len(self.field_buffers):
                    ys = list(self.field_buffers[i])
                    # align lengths defensively
                    n = min(len(xs), len(ys))
                    if n > 0:
                        self.ax.plot(xs[-n:], ys[-n:], label=field_label(i))
                        any_plotted = True
